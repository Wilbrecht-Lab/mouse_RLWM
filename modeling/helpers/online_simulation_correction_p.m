function data_sim = online_simulation_correction_p(data, param)
...
% I am trying to write an independent script to debug.

% This script is used for implementing stickiness at the policy level.
...
for sess = 1:num_sess
    ...
    for trial = 1:num_trial
        if action_history(trial) > 0 % not a missing trial
            ...
            % calculate policy first
            pchoice(1) = 1 / (1 + exp(beta(sess) * (Q(stim, 2) - Q(stim, 1))));
            ...
            if r_sim ~= 1 % not rewarded, i.e. get it wrong
                ...
                if correction_trial < num_trial
                    stim_history_sim(correction_trial) = stim; % update the odor of a near future trial online
                end
            end
