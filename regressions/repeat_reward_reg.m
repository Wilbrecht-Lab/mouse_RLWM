function  [regression_coeff, regression_SE, regression_tStat, regression_pValue, regression_modelcomparison] = repeat_reward_reg(data, filler)
...
for sess = 1:num_sess
    ...
    temp_together(:, 5) = log(temp_together(:, 5) + 1); % correct history
...
regression_modelcomparison = zeros(num_sess,6); % AIC, AICc, BIC, CAIC

for sess = 1:num_sess
%     [regression_coeff(sess, :), ~, ~] = glmfit(together{sess}(:, 1:5), together{sess}(:, 6), 'binomial', 'link', 'logit');
    mdl = fitglm(together{sess}(:, 1:5), together{sess}(:,6),'Distribution','binomial') ;
