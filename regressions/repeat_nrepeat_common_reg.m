function [regression_coeff, regression_SE, regression_tStat, regression_pValue, regression_modelcomparison, corr_map, repeat_proportion] = repeat_nrepeat_common_reg(data, filler)
...
regression_coeff = zeros(num_sess, 5);% 5 means the coeff for intercept, one-back, reg1, reg2 and correct history
...
for sess = 1:num_sess
    ...
    for trial = 1:num_trial
        ...
        if (trial > 1)  
            ...
            if action_history(trial - 1) > 0 && a > 0
                ...
                reg_sess = [reg_sess; one_back, one_back_repeat, one_back_nrepeat, correct_history(s), r];
            end
        end
    ...
    reg_sess(:, 4) = log(reg_sess(:, 4) + 1); % log transform correct history (more satiated as experiment progresses)
